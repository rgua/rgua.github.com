...
Снова воспользуемся генератором rails для того чтобы сгенерировать стили, и применить их к готовым страничкам
...
Сгенерируем всё необходимое для работы с подарками и обновим таблицы в базе данных

{% highlight sh %}
rails generate scaffold Gift description:text user:references
rake db:migrate
{% endhighlight %}

Откройте config/routes.rb и замените строки
...
{% highlight sh %}
rails generate bootstrap:themed Gifts
{% endhighlight %}
здесь выберите
{% highlight sh %}
a
...
Ещё было бы удобно добавить ссылочку на подарки пользователей в список:
...
Каждый приглашённый пользователь должен будет получить e-mail с приглашением на сайт. Для этого нужно настроить почтовый сервер. Мы воспользуемся сервисом gmail
